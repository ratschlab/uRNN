...
# --- IRNN --- #
IRNN_trace<-read.table(paste0(base_dir, "/T400/lr1e-4_IRNN_T400_n80.vali.txt"), header=TRUE)
batch_size<-20
batch_skip<-150
...
# --- LSTM --- #
LSTM_trace<-read.table(paste0(base_dir, "/T400/LSTM_T400_n40.vali.txt"), header=TRUE)
batch_size<-20
batch_skip<-150
...
# --- tanhRNN --- #
tanhRNN_trace<-read.table(paste0(base_dir, "/T400/tanhRNN_T400_n80.vali.txt"), header=TRUE)
batch_size<-20
batch_skip<-150
...
# --- NOW FOR PLOT --- #
ggplot(da, aes(x=num_updates, y=cost, group=which, colour=which)) + geom_point(cex=0.3) + geom_line(alpha=0.2) + coord_cartesian(ylim=c(0, 0.25)) + ggtitle("adding T = 400")
ggsave(paste0(base_dir, "/adding_T400.png"), width=4.5, height=3)
